...
const RestaurantDisplay = () => {
    ...
    const approvedRestaurants = restaurant_list.filter(item => item.isApproved);
    ...
    return (
        <div className='restaurant-display px-[5vw] py-16 bg-[#fcfdfe]' id='restaurant-display'>
            ...
            {/* --- Restaurant Grid --- */}
            <div className='restaurant-display-list grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-12'>
                {restaurant_list.map((item, index) => {
                    const isApproved = item.isApproved === true;

                    // Search Filtering
                    if (searchQuery && !item.name.toLowerCase().includes(searchQuery.toLowerCase()) && !item.description?.toLowerCase().includes(searchQuery.toLowerCase())) return null;

                    // Comprehensive Collection Filtering
                    if (selectedCollection !== "All") {
                        if (selectedCollection === "Gourmet Dining" && (item.rating || 0) < 4.5) return null;
                        if (selectedCollection === "Express Delivery" && item.name.toLowerCase().includes("bistro")) return null; // Mock logic
                        if (selectedCollection === "Healthy Eats" && !item.description?.toLowerCase().includes("organic") && !item.description?.toLowerCase().includes("fresh")) return null;
                        if (selectedCollection === "Global Flavors" && !item.name.toLowerCase().includes("italia") && !item.name.toLowerCase().includes("sushi")) return null;
                    }
                    ...
                    return (
                        <div
                            ...
                            className={`restaurant-card group relative bg-white rounded-[32px] overflow-hidden transition-all duration-500 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] ${isApproved ? 'hover:-translate-y-3 cursor-pointer' : 'opacity-80 grayscale cursor-not-allowed'} animate-fadeIn`}
